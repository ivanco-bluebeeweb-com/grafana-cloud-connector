"""Connection lifecycle for Grafana Cloud Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from grafana_cloud_client import GrafanaCloudClient
...
_SECRET = "grafana_cloud_connections"
...
@chat.function(
    "connect_grafana_cloud",
    "Connect Grafana Cloud account via credentials.",
    action_type="write",
    chain_callable=True,
    event="grafana-cloud-connector.connect_grafana_cloud",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_grafana_cloud(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Grafana Cloud Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "Grafana Cloud Account",
...
@chat.function(
    "list_connections",
    "List connected Grafana Cloud accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_grafana_cloud",
    "Disconnect Grafana Cloud account.",
    action_type="write",
    chain_callable=True,
    event="grafana-cloud-connector.disconnect_grafana_cloud",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_grafana_cloud(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
